...
Because of those last two, every state in this extension — including "no artifacts" and "no matches" — offers **Open Code Gallery** (⌘⇧O) and **Open Chat Gallery** (⌘⇧G), which jump to `claude.ai/code/artifacts` and `claude.ai/artifacts`. When an artifact isn't in the index, it usually isn't missing — it was published somewhere the hook can't see, and the gallery is where it actually lives.
